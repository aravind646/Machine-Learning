...
%    [cost, grad, hess] = costLR(A, B, C); % hessian is optional
    ...
    %disp('X dash')
    %size(X')
    sigmoidValue = sigmoid(theta' * X');        
    regTerm = (lambda / 2) * (sumsqr(theta));
    %disp('regTerm')
    ...
    modTerm =  (log(sigmoidValue) * y) + (log(1 - sigmoidValue) * (1 - y));
    cost = regTerm - modTerm;    
    ...
    gradientModTerm = (y - sigmoidValue)' * X;
    %disp('gradientModTerm')
    %size(gradientModTerm)
    
    gradientRegTerm = lambda * theta;
    %disp('gradientRegTerm')
    %size(gradientRegTerm)
            
    grad = gradientRegTerm - gradientModTerm';
    %disp('grad after')
    %size(grad)    
    %% END SOLUTION
end
