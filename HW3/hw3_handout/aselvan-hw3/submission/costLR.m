...
%    [cost, grad, hess] = costLR(A, B, C); % hessian is optional
    ...
    costRegTerm = (lambda / 2) * (sum(sumsq(theta)));
    costTerm =  (log(sigmoidValue) * y) + (log(1 - sigmoidValue) * (1 - y));
    cost = costRegTerm - costTerm;    
    sigmoidValue = sigmoid(X * theta);        
    gradientTerm = (y - sigmoidValue)' * X;
    gradientRegTerm = lambda * theta;
    grad = gradientRegTerm - gradientTerm';
    %% END SOLUTION
end
